...
# Convert the final data (or the relevant part) to CSV format
@st.cache_data
def convert_df_to_csv(df):
    # IMPORTANT: The to_csv() method handles the conversion
    return df.to_csv(index=True)

# Create a DataFrame for download (e.g., combining portfolio returns)
download_df = pd.DataFrame({
    'High_Quality_Returns': cum_high,
    'Low_Quality_Returns': cum_low,
    'High_Low_Ratio': cum_high / cum_low
})

csv = convert_df_to_csv(download_df)

st.download_button(
    label="Download Portfolio Performance as CSV",
    data=csv,
    file_name='quality_anomaly_results.csv',
    mime='text/csv',
)
